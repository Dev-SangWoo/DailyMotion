...
- [x] 혼잡도 데이터 소스 (1단계: 역/구간 단위 평균 혼잡도)
  ...
  - [ ] `RecommendedTransport`에 혼잡도 필드 추가 여부 논의 (예: `congestionLevel`, `congestionValue`)
...
- [ ] 실시간 실패 시 Fallback 순서 명시
  ...
  - [ ] 각 단계별 “사용 가능/사용 불가” 기준 정의 (예: 통계 샘플수 < N이면 사용 안 함)
- [ ] 실패 시 메시지 정책
  - [ ] ETA가 Fallback만 가능한 경우, 사용자에게 어느 정도까지 노출할지 결정
    - [ ] 예: “실시간 정보를 불러올 수 없어 평균값 기반으로 계산되었습니다.”

**사용자가 제공/확인해 줄 것**
- [ ] “통계 데이터 신뢰도 기준”(샘플 개수/최신 업데이트 시점 등)에 대한 정책
...
- [ ] 환승 포인트에서의 실시간 대기시간 계산
  - [ ] 지하철 환승: 도착역/환승역 기준 다음 열차 ETA 조회
  - [ ] 버스 환승: 환승 정류장 기준 다음 버스 ETA 조회
  - [ ] ODSay `sectionTime`과 실시간 ETA를 조합해 “환승 여유 시간(≥ 3분)”을 더 정확히 계산
- [ ] Logic 2.2 Gate 2(환승 확정성) 개선
  - [ ] 현재 상수/단순 계산 부분을 “실시간 환승 대기시간”으로 교체
